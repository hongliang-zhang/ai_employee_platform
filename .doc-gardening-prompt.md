...
| `docs/QUALITY_SCORE.md` | `packages/` 下 `*.test.ts` 文件 | 读文档中描述的测试文件列表，在代码库中确认对应 `*.test.ts` 文件是否存在；覆盖率评级是否与测试文件的实际存在情况相符 |
...
| `docs/exec-plans/tech-debt-tracker.md` | `packages/*/src/` | 读文档中每条 tech debt 条目，在代码库中判断该问题是否已被解决，将已解决的条目标记完成 |
| `docs/` 所有 `.md` 中的相对链接 | 被引用路径 | 读每个 .md 文件中的相对链接，确认目标文件或目录是否存在。链接有效性是结构性检查，没有判断模糊地带——链接存在则有效，不存在则修复，不适用"无法判断时"协议。 |
...
- **不做修改**，不强行归档或更新状态
- 若同一文件中部分内容可以确认、部分无法判断，修改可以确认的部分，对无法确认的部分加 FLAG，不必因为存在不确定项而放弃整个文件的修改。
...
- 不确定时，加 `<!-- DOC-GARDENING-FLAG: [描述问题] -->` 标注（格式见"无法判断时的处理"章节）；若同一文件同时有 CHANGE 和 FLAG，CHANGE 块在前，FLAG 块紧随其后
