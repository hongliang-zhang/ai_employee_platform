...
| Step 1 全勾，但 Step 3 文件全不存在 | 不归档：加 `<!-- DOC-GARDENING-FLAG: checkboxes 全勾但文件不存在，需人工确认 -->` |
| 其他 | 不归档 |
...
| Draft 或 Active | 对应 exec plan 已归档到 `docs/exec-plans/completed/` | Completed |
| Draft 或 Active | 无对应 exec plan，且 git log 有 ≥1 相关 commits | Completed |
| 任何状态 | 信号不一致或无法判断 | 不修改，加 `<!-- DOC-GARDENING-FLAG: [描述问题] -->` 标注 |
